<template>
    ...
    infinite-scroll-distance="10">
			<router-link class="order-item" v-for="(item,index) in orderData" :key="index" :to="{ path: '/delivery/order/detail', query: { orderNo: item.orderNo } }" tag="div">
				<div class="status">
					<span>状态</span>
		        ...
		        <svg class="icon" aria-hidden="true">
				  ...
				  <div class="part-default">
				    <div class="arrive text-large">{{ item.reciever }}，{{ item.mobile }}</div>
				    <div class="address-detail">{{ item.addr }}</div>
				  </div>
				</div>
				<div class="to-deliver" v-if="item.status == '已付款'">
					<span @click.stop.prevent="toDeliver(item.orderNo)">发货</span>
				</div>
			</router-link>
    </div>
    <!-- 无数据 -->
...
export default {
    ...
    loadMore () {
      if (!this.nomore) {     
		...
		toDeliver (orderNo) {
			this.$router.push({ path: '/delivery/detail', query: { orderNo } })
		},
		toggleOrderTab (type) {
			this.type = type
			this.page = 1
			this.orderData = []
			this.$router.push({ path: '/delivery/order', query: { type: type } })
		},
		getOrderList (type) {
			let method = type;
			// 初始化数据
	    ...
	    let params = {
			...
			http.get('/supplier/' + method, params, false, res => {
